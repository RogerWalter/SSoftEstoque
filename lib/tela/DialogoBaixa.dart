...
import 'package:ssoft_estoque/util/ItemController.dart';
import '../model/Item.dart';
import '../util/Controller.dart';
import '../util/Util.dart';

class DialogoBaixa extends StatefulWidget {
  const DialogoBaixa({Key? key}) : super(key: key);
...
class _DialogoBaixaState extends State<DialogoBaixa> {
                          ...
                          Observer(
                              builder: (_){
                                return Container(
                                    height: 25,
                                    child: Switch(
                                      value: controller_mobx.baixa_parametro_camera,
                                      activeColor: cores.laranja_teccel,
                                      onChanged: (bool value){
                                        controller_mobx.baixa_parametro_camera = value;
                                      },
                                    )
                                );
                              }
                          ...
                          Expanded(
                            child: Container(
                              ...
                              child: TextField(
                                ...
                                onChanged: (content){
                                  if(content.length == 9){
                                    controller_mobx.baixa_buscar_item_lista_estoque(content);
                                    _controller_nome.text = controller_mobx.baixa_item_lista_estoque.nome;
                                    _controller_unidade.text = controller_mobx.baixa_item_lista_estoque.unidade;
  ...
  Future<void> scan_codigo() async {
    ...
    if(barcodeScanRes == "-1")
      return;
    controller_mobx.baixa_retorno_scan = await barcodeScanRes;
    _controller_codigo.text = await controller_mobx.baixa_retorno_scan;
    if(_controller_codigo.text.length == 9){
      await controller_mobx.baixa_buscar_item_lista_estoque(_controller_codigo.text);
      _controller_nome.text = controller_mobx.baixa_item_lista_estoque.nome;
      _controller_unidade.text = controller_mobx.baixa_item_lista_estoque.unidade;
      if(_controller_unidade.text != ""){
        _foco_qtd.requestFocus();
  ...
  salvar_item(BuildContext thiscontext){
    if(_controller_codigo.text.isEmpty || _controller_codigo.text.length < 9
      ...
      if(!verificar_qtd()){
        ...
        Flushbar(
          backgroundColor: cores.cor_accent,
          message: 'Não há estoque suficiente para baixar a quantidade desejada deste item. Altere a quantidade e tente novamente.',
          duration: Duration(seconds: 2),
        ).show(context);
      }
      else{
        int indice_busca = -1;
        indice_busca = controller_mobx.lista_itens_baixa.indexWhere((element) => element.codigo == _controller_codigo.text);
        if(indice_busca != -1){
          Flushbar(
            backgroundColor: cores.cor_accent,
            message: 'Este item ja foi inserido na listagem. Cada item deve aparecer uma unica vez na baixa. Se deseja alterar a quantidade, exclua o item e insira-o novamente',
            duration: Duration(seconds: 2),
          ).show(context);
        }
        else{
          if(controller_mobx.lista_separacoes.length > 0){//verificação pra calcular se a inserção do item é possível, somando as quantidades que estão "separadas" com o que tem disponível no estoque
            int qtd_baixa = int.parse(_controller_qtd.text);
            int qtd_atual = controller_mobx.baixa_item_lista_estoque.qtd;
            int qtd_separada = 0;
            for(DadosSeparacao separacao in controller_mobx.lista_separacoes){
              for(ItemSeparacao item in separacao.itens){
                if(item.codigo == _controller_codigo.text)
                  qtd_separada = qtd_separada + item.qtd;
              }
            }
            int resultado = qtd_atual - qtd_baixa - qtd_separada;
            if(resultado < 0){
              Flushbar(
                backgroundColor: cores.cor_accent,
                message: 'Existem quantidades separadas deste item. A quantidade que está tentando inserir irá negativar o estoque. Exclua as separações em aberto ou dê entrada em novas quantidades para conseguir inserir este item.',
          ...
          item_add.controla_serial = controller_mobx.baixa_item_lista_estoque.controla_serial;
          if(item_add.controla_serial == 1){
            ...
            controller_mobx.lista_seriais_itens_baixa.add(serial_add);
          }
          controller_mobx.adiciona_item_baixa(item_add);
          controller_mobx.alterar_arametro_houve_alteracao(true);
          if(controller_mobx.baixa_parametro_camera){
            scan_codigo();
          }
          else{
            Navigator.of(context).pop();
  ...
  bool verificar_qtd(){
    ...
    int qtd_baixa = int.parse(_controller_qtd.text);
    int qtd_atual = controller_mobx.baixa_item_lista_estoque.qtd;
    int resultado = qtd_atual - qtd_baixa;
    if(resultado >= 0){
      //estoque suficiente para a baixa
      retorno = true;
    }
    else{
      //estoque insuficiente para a baixa
      retorno = false;
    }
    return retorno;
  }
